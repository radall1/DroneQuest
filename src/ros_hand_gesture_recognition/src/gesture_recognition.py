...
class GestureRecognition:
    ...
    def recognize(self, image, time_since, number=-1, mode=0):
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                # Drawing part
                debug_image = self._draw_bounding_rect(USE_BRECT, debug_image, brect, time_since)
    ...
    def _draw_bounding_rect(self, use_brect, image, brect, time_since):
        if use_brect:
            # Outer rectangle
            ...
            radius = min((brect[2] - brect[0]) // 2, (brect[3] - brect[1]) // 2)

            # Calculate the angle parameters for 3/4 of a circle
            start_angle = 0  # Start angle in degrees (0 degrees)
            end_angle = 360/1.75  # End angle in degrees (3/4 of a circle, 270 degrees)

            # Draw a green arc representing 3/4 of the circle
            cv.ellipse(image, center, (radius, radius), 0, start_angle, time_since*end_angle, (0, 255, 0), 2)
